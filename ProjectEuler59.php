...
function findAsciiSum()
{
    $file = __DIR__ . '/docs/problem59.txt';
    $charArr = explode(',', file_get_contents($file));
    $decryptedText = decryptText($charArr);
    $sum = getAsciiSum($decryptedText);
    return $sum;
}

function decryptText($charArr)
{
    foreach (range('a', 'z') as $char1) {
        foreach (range('a', 'z') as $char2) {
            foreach (range('a', 'z') as $char3) {
                $key = array(ord($char1), ord($char2), ord($char3));
                $decryptedStr = decryptWithKey($charArr, $key);
                if (stripos($decryptedStr, '.') && stripos($decryptedStr, ' ') && stripos($decryptedStr, 'the') && stripos($decryptedStr, 'be')
                    && stripos($decryptedStr, 'to') && stripos($decryptedStr, 'of') && stripos($decryptedStr, 'and')) {
...
function decryptWithKey($charArr, $key)
{
    $decrypted = array();
    $decryptedStr = '';
    $i = 0;
